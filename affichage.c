...
/*   Updated: 2024/04/26 14:40:58 by hsolet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "so_long.h"
...
static void	put_other(t_game	*s)
{
	if (s->map[s->i][s->j] == 'E')
		mlx_put_image_to_window(s->mlx,
			s->mlx_win, s->img->exit, s->j * 90, s->i * 90);
	if (s->map[s->i][s->j] == 'P')
		mlx_put_image_to_window(s->mlx, s->mlx_win,
			s->img->player, (s->j * 90) + 20, (s->i * 90) + 20);
	if (s->map[s->i][s->j] == 'C')
		mlx_put_image_to_window(s->mlx, s->mlx_win,
			s->img->col, (s->j * 90) + 30, (s->i * 90) + 30);
...
void	put_decor(t_game	*s)
{
	...
	while (s->map[s->i])
	{
		...
		{
			mlx_put_image_to_window(s->mlx,
				s->mlx_win, s->img->ground, s->j * 90, s->i * 90);
			if (s->map[s->i][s->j] == '1')
				mlx_put_image_to_window(s->mlx,
					s->mlx_win, s->img->wall, s->j * 90, s->i * 90);
			put_other(s);
			s->j++;
		}
		s->i++;
